...
\]
where $\mu_\delta$ is the non-atom mean contrast and $\beta_\delta$ is the log odds-ratio for escaping the atom. A numerical combined-outcome contrast, denoted $\Delta$ or $\Delta^{(e_0)}$, is a derived scalar functional obtained only after the atom is assigned the numerical value $e_0$ through $c_{e_0}$. Thus component-wise inference for $\theta_\delta$ and scalar inference for $\Delta^{(e_0)}$ address related but distinct questions. Throughout, $\chi^2_{q,1-\alpha}$ denotes the $(1-\alpha)$ quantile of a chi-square distribution with $q$ degrees of freedom.
...
As a scalar benchmark, one may ignore the two-component decomposition and analyze the coded outcome $\widetilde{Y}^{(e_0)}=c_{e_0}(\widetilde{Y})$ as an ordinary real-valued response. Let $\widehat{\Delta}_{\mathrm{W}}$, $\widehat{\mathrm{se}}_{\mathrm{W}}$, and $\widehat{\nu}$ denote the usual two-sample Welch estimate, standard error, and degrees of freedom computed from $\widetilde{Y}^{(e_0)}$, and let $t_{\widehat{\nu},1-\alpha/2}$ be the corresponding Student quantile. The Welch interval is
\begin{equation}
...
Let
\(
  \mathcal{D}_n=\{(\widetilde{Y}_i^{(e_0)},A_i,R_i):i=1,\ldots,n\}
\)
...
Then, for measurable \(B\subseteq\{e_0\}\cup\mathcal{C}\), the posterior predictive distribution of a future numerical combined outcome is
\[
  ...
  \left\{
    \rho_r\delta_{e_0}(B)
    +\pi_r F_r^c(B\cap\mathcal{C}\mid\vartheta)
  \right\}
  p(\vartheta\mid\mathcal{D}_n)\,d\vartheta .
\]
For continuous-support fits, \(F_r^c(dy\mid\vartheta)=f_r^c(y\mid\vartheta)\lambda_{\mathcal{C}}(dy)\), with
\[
...
\[
  \mu_r^{c,(m)}
  =
  \int_{\mathcal{C}} y\,F_r^c(dy\mid\vartheta^{(m)}),
\]
...
For the atom-at-zero representation used elsewhere in the manuscript, \(\Delta=\Delta^{(0)}=\pi_1\mu_1^c-\pi_0\mu_0^c\).
...
\[
  \rho_r(x)=1-\pi_r(x),
  \qquad
  \mu_r^c(x)=\int_{\mathcal C}y\,F_r^c(dy\mid x),
  \qquad
