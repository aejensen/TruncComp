...
bayes_validate_heaping_grids <- function(heaping_grids, score_step,
  ...
  range_multiples <- vapply(
    heaping_grids,
    function(grid) bayes_is_grid_multiple(score_max - score_min, grid, tol),
    logical(1)
  )

  if(!all(range_multiples)) {
    stop("score_max - score_min must be an integer multiple of every value in heaping_grids.")
  }
...
bayes_validate_bounded_data <- function(data, atom, support_options) {
  ...
  if(identical(support_options$continuous_support, "bounded_continuous")) {
    if(any(observed <= score_min | observed >= score_max)) {
      stop(
        "For continuous_support = \"bounded_continuous\", non-atom outcomes must be strictly inside (score_min, score_max). Use the bounded_score model when boundary values occur."
      )
    }
