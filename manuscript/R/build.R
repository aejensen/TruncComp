...
.application_bayes_diagnostic_macros <- function(bayes_result) {
  diagnostics <- application_bayes_sampled_parameter_diagnostics(bayes_result$fit)

  list(
    ApplicationBayesMaxRhat = format_fixed(diagnostics$max_rhat, 4),
    ApplicationBayesMinBulkEss = format_count(diagnostics$min_bulk_ess),
    ApplicationBayesMinTailEss = format_count(diagnostics$min_tail_ess)
  )
}
...
.application_value_macros <- function(application_results) {
  ...
  delta_interval_bound <- function(method, bound) {
    interval <- application_results$delta_intervals
    row <- interval[interval$method == method, , drop = FALSE]
    value <- if (nrow(row) == 1L && identical(row$status[[1]], "estimated")) {
      row[[bound]][[1]]
    } else {
      NA_real_
    }
    if (is.finite(value)) format_number(value, 2) else "not estimated"
  }
  ...
  c(
    list(
      ...
      ApplicationDeltaWelchLower = delta_interval_bound("welch", "lower"),
      ApplicationDeltaWelchUpper = delta_interval_bound("welch", "upper"),
      ApplicationDeltaProfileLower = delta_interval_bound("profile", "lower"),
      ApplicationDeltaProfileUpper = delta_interval_bound("profile", "upper"),
      ApplicationDeltaProjectedLower = delta_interval_bound("projected", "lower"),
      ApplicationDeltaProjectedUpper = delta_interval_bound("projected", "upper"),
      ApplicationTTestP = format_p_value(.application_test_p(application_results, "Coded endpoint", "Welch t-test")),
      ApplicationWilcoxP = format_p_value(.application_test_p(application_results, "Coded endpoint", "Wilcoxon rank-sum")),
    ...
    .application_bayes_diagnostic_macros(bayes)
  )
}
