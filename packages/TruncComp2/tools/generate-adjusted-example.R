...
script_path <- if(length(script_arg) > 0) {
  ...
  normalizePath("tools/generate-adjusted-example.R", winslash = "/", mustWork = FALSE)
}
...
output_path <- if(length(args) > 0) args[1] else {
  file.path(package_root, "inst", "extdata", "TruncComp2AdjustedExample.rds")
}
...
adjusted_example <- generate_adjusted_example()
dir.create(dirname(output_path), recursive = TRUE, showWarnings = FALSE)
saveRDS(adjusted_example, output_path, version = 2)

cat("Saved adjusted example to", output_path, "\n")
