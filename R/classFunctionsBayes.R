bayes_contrast_labels <- function() {
  c(
    ...
    mu_delta = "Conditional non-atom mean difference",
    alpha_delta = "Odds ratio for being outside the atom",
    delta = "Coded endpoint mean difference (delta)"
  )
}
...
bayes_arm_labels <- function() {
  c(
    ...
    pi_0 = "Probability outside atom, control",
    pi_1 = "Probability outside atom, treatment",
    mu_0_c = "Conditional non-atom mean, control",
    mu_1_c = "Conditional non-atom mean, treatment"
  )
}
...
summary.trunc_comp_bayes_fit <- function(object, ...) {
  ...
  if(!is.null(object$settings$auto_select_mixture_components)) {
    cat("Sampler-diagnostic component refitting =",
  ...
  if(!is.null(diagnostics$truncation)) {
    cat("  Mixture-parameter sampler diagnostics OK:", isTRUE(diagnostics$truncation_ok), "\n")
  }
...
#'   treatment-minus-control difference in the conditional non-atom mean,
#'   `alpha_delta` is the treatment-to-control odds ratio for being outside the
#'   atom, and `delta` is the coded endpoint mean difference induced by the
#'   specified atom code.
#' @export
