...
parametric_profile_design <- function(fit, term) {
  if(is.null(fit)) {
    return(NULL)
  }
  ...
  if(is.null(model_frame) || is.null(design) ||
     !(term %in% colnames(design))) {
    return(NULL)
  }

  response <- tryCatch(stats::model.response(model_frame), error = function(e) NULL)
  if(is.null(response) || NROW(response) != nrow(design)) {
    return(NULL)
  }
  ...
  base_offset <- stats::model.offset(model_frame)
  if(is.null(base_offset)) {
    base_offset <- rep(0, nrow(design))
  }

  term_index <- match(term, colnames(design))
  list(
    response = response,
    reduced_design = design[, -term_index, drop = FALSE],
    term_design = as.numeric(design[, term_index]),
    weights = as.numeric(weights),
    base_offset = as.numeric(base_offset)
  )
}

parametric_lm_profile_interval <- function(fit, term, conf.level,
                                           transform = identity) {
  estimate <- parametric_term_estimate(fit, term)
  covariance_scale <- tryCatch(summary(fit)$cov.unscaled, error = function(e) NULL)
  rss <- tryCatch(stats::deviance(fit), error = function(e) NA_real_)
  n <- tryCatch(stats::nobs(fit), error = function(e) NA_integer_)

  if(is.null(covariance_scale) || !(term %in% rownames(covariance_scale)) ||
     !is.finite(estimate) || !is.finite(rss) || rss <= 0 ||
     !is.finite(n) || n <= 0) {
    return(c(NA_real_, NA_real_))
  }

  scale <- as.numeric(covariance_scale[term, term])
  if(!is.finite(scale) || scale <= 0) {
    return(c(NA_real_, NA_real_))
  }
  critical_value <- stats::qchisq(conf.level, df = 1)
  half_width <- sqrt(scale * rss * expm1(critical_value / n))
  interval <- transform(estimate + c(-1, 1) * half_width)
...
parametric_lm_profile_fit <- function(fit, term, value,
                                      ll_unrestricted = NULL,
                                      tol = 1e-12) {
  profile_design <- parametric_profile_design(fit, term)
  if(is.null(profile_design) || !is.finite(value)) {
    return(list(fit = NULL, statistic = Inf))
  }

  rss_unrestricted <- tryCatch(stats::deviance(fit), error = function(e) NA_real_)
  n <- tryCatch(stats::nobs(fit), error = function(e) NA_integer_)
  if(!is.finite(rss_unrestricted) || rss_unrestricted <= 0 ||
     !is.finite(n) || n <= 0) {
    return(list(fit = NULL, statistic = Inf))
  }

  response <- profile_design$response - profile_design$base_offset -
    value * profile_design$term_design
  constrained <- suppressWarnings(tryCatch(
    stats::lm.wfit(
      x = profile_design$reduced_design,
      y = response,
      w = profile_design$weights
    ),
    error = function(e) NULL
  ))
  if(!is.null(constrained)) {
    names(constrained$coefficients) <- colnames(profile_design$reduced_design)
  }
  rss_constrained <- if(is.null(constrained)) {
    NA_real_
  } else {
    sum(profile_design$weights * constrained$residuals ^ 2)
  }
  statistic <- if(is.finite(rss_constrained) && rss_constrained > 0) {
    parametric_clamp_statistic(
      n * log(rss_constrained / rss_unrestricted),
      tol = tol
    )
  } else {
    Inf
  ...
  list(fit = constrained, statistic = statistic)
}

parametric_glm_profile_statistic <- function(fit, term, value,
                                             ll_unrestricted = NULL,
                                             tol = 1e-12) {
  profile_design <- parametric_profile_design(fit, term)
  if(is.null(profile_design) || !is.finite(value)) {
  ...
  if(is.null(ll_unrestricted)) {
    ll_unrestricted <- parametric_loglik_value(fit)
  }
  if(!is.finite(ll_unrestricted)) {
    return(Inf)
  }
  ...
  constrained <- suppressWarnings(tryCatch(
    stats::glm.fit(
      x = profile_design$reduced_design,
      y = profile_design$response,
      weights = profile_design$weights,
      offset = profile_design$base_offset +
        value * profile_design$term_design,
      family = fit$family,
  ...
  if(!is.null(constrained)) {
    class(constrained) <- c("glm", "lm")
  }
  ll_constrained <- parametric_loglik_value(constrained)
  if(!is.finite(ll_constrained)) {
    return(Inf)
  }

  parametric_clamp_statistic(
    2 * (ll_unrestricted - ll_constrained),
    tol = tol
  )
}

parametric_glm_profile_interval <- function(fit, term, conf.level,
                                            transform = identity,
                                            tol = 1e-8,
                                            max_expand = 60L) {
  estimate <- parametric_term_estimate(fit, term)
  ...
  initial_step <- max(sqrt(variance) * stats::qnorm((1 + conf.level) / 2), 0.25)
  ...
  find_endpoint <- function(direction) {
    ...
    for(i in seq_len(max_expand)) {
      ...
      value <- suppressWarnings(objective(candidate))
    ...
    if(!is.finite(outside)) {
      return(NA_real_)
    }

    interval <- sort(c(inside, outside))
  ...
  interval <- c(find_endpoint(-1), find_endpoint(1))
...
parametric_normal_summary <- function(y0, y1, fits, conf.level, tol = 1e-12) {
  ...
  if(sse1 <= tol) {
    ...
    W <- 2 * (ll_ha - ll_h0)
    W <- parametric_clamp_statistic(W, tol = tol)
    muDeltaCI <- parametric_lm_profile_interval(fits$normal_alt, "R", conf.level)
  } else {
...
parametric_adjusted_lrt_fit <- function(data, adjust, conf.level = 0.95,
  ...
  alpha_delta_ci <- parametric_glm_profile_interval(
    fits$bernoulli_alt,
    "R",
    conf.level,
    transform = exp
  )
  mu_delta_ci <- parametric_lm_profile_interval(fits$normal_alt, "R", conf.level)
...
LRT <- function(data, conf.level = 0.95, adjust = NULL, adjust_spec = NULL,
                atom = NULL, call = NULL) {
