...
parametric_component_profile_statistics <- function(data, fit, adjust = NULL) {
  formulas <- TruncComp:::parametric_model_formulas(adjust)
  observed_variables <- c("Y", "R")
  if(!is.null(adjust)) {
    observed_variables <- unique(c(observed_variables, all.vars(adjust)))
  }
  observed <- droplevels(data[data$A == 1, observed_variables, drop = FALSE])

  glm_alt <- suppressWarnings(stats::glm(
    formulas$bernoulli_alt,
    family = stats::binomial(),
    data = data
  ))
  lm_alt <- stats::lm(formulas$normal_alt, data = observed)

  alpha_statistics <- vapply(log(fit$alpha_delta_ci), function(value) {
    constrained_data <- data
    constrained_data$.trunccomp_profile_offset <- value * data$R
    constrained <- suppressWarnings(stats::glm(
      formulas$bernoulli_null,
      family = stats::binomial(),
      data = constrained_data,
      offset = .trunccomp_profile_offset
    ))
    2 * (as.numeric(stats::logLik(glm_alt)) - as.numeric(stats::logLik(constrained)))
  }, numeric(1))

  mu_statistics <- vapply(fit$mu_delta_ci, function(value) {
    constrained_data <- observed
    constrained_data$.trunccomp_profile_offset <- value * observed$R
    constrained <- stats::lm(
      formulas$normal_null,
      data = constrained_data,
      offset = .trunccomp_profile_offset
    )
    2 * (as.numeric(stats::logLik(lm_alt, REML = FALSE)) -
      as.numeric(stats::logLik(constrained, REML = FALSE)))
  }, numeric(1))

  list(alpha = alpha_statistics, mu = mu_statistics)
}

expect_parametric_component_profile_endpoints <- function(data, fit, adjust = NULL,
                                                          conf.level = 0.95,
                                                          tolerance = 1e-5) {
  critical_value <- stats::qchisq(conf.level, df = 1)
  statistics <- parametric_component_profile_statistics(data, fit, adjust = adjust)

  expect_equal(statistics$alpha, rep(critical_value, 2), tolerance = tolerance)
  expect_equal(statistics$mu, rep(critical_value, 2), tolerance = tolerance)
  expect_lte(fit$alpha_delta_ci[1], as.numeric(fit$alpha_delta))
  expect_gte(fit$alpha_delta_ci[2], as.numeric(fit$alpha_delta))
  expect_lte(fit$mu_delta_ci[1], fit$mu_delta)
  expect_gte(fit$mu_delta_ci[2], fit$mu_delta)
}
...
optim_parametric_lrt <- function(data, adjust = NULL) {
  fits <- TruncComp:::parametric_fit_models(data, adjust = adjust)
...
test_that("model-backed parametric engine matches direct glm/lm references on regular data", {
  ...
  for(case in cases) {
    fit <- TruncComp:::parametric_lrt_fit(case)
    ...
    expect_parametric_component_profile_endpoints(case, fit)
    expect_equal(fit$p.value, stats::pchisq(fit$statistic, df = 2, lower.tail = FALSE), tolerance = 1e-12)
    expect_gte(fit$statistic, 0)
  }
})

test_that("parametric component intervals invert the requested likelihood ratio cutoff", {
  case <- interior_lrt_case(20260414, 18, 2.1, 2.8, 0.9, 0.45, 0.70)
  conf.level <- 0.90
  fit <- TruncComp:::parametric_lrt_fit(case, conf.level = conf.level)

  expect_true(fit$success)
  expect_parametric_component_profile_endpoints(
    case,
    fit,
...
test_that("adjusted parametric engine matches direct glm/lm references", {
  ...
  for(case in cases) {
    fit <- TruncComp:::parametric_lrt_fit(case$data, adjust = case$adjust)
    ...
    expect_equal(fit$alpha_delta, ref$alpha_delta, tolerance = 1e-10)
    expect_equal(fit$mu_delta, ref$mu_delta, tolerance = 1e-10)
    expect_parametric_component_profile_endpoints(case$data, fit, adjust = case$adjust)
...
test_that("public LRT path returns the expected object", {
...
test_that("public interfaces reject unsupported arguments", {
  case <- interior_lrt_case(20260424, 12, 2.4, 3.0, 0.9, 0.50, 0.70)
...
test_that("public interfaces validate the finite atom and binary indicators", {
  case <- interior_lrt_case(20260424, 12, 2.4, 3.0, 0.9, 0.50, 0.70)

  expect_error(
    trunc_comp(Y ~ R, atom = Inf, data = case[, c("Y", "R")], method = "lrt"),
    "finite numeric"
  )
  expect_error(
    trunc_comp(case$Y, c(case$A[-1], 2), case$R, atom = 0, method = "lrt"),
    "a must be binary"
  )
  expect_error(
    trunc_comp(case$Y, case$A, rep(0, length(case$R)), atom = 0, method = "lrt"),
    "both treatment groups"
  )
  expect_error(
    trunc_comp(case$Y, case$A, c(case$R[-1], 2), atom = 0, method = "lrt"),
    "r must contain both treatment groups"
  )
})

test_that("logical treatment indicators are normalized to numeric model terms", {
  case <- interior_lrt_case(20260424, 12, 2.4, 3.0, 0.9, 0.50, 0.70)
  logical_a <- as.logical(case$A)
  logical_r <- as.logical(case$R)

  for(method in c("lrt", "splrt")) {
    fit <- trunc_comp(case$Y, logical_a, logical_r, atom = 0, method = method)
    formula_data <- data.frame(Y = case$Y, R = logical_r)
    formula_fit <- trunc_comp(
      Y ~ R,
      atom = 0,
      data = formula_data,
      method = method
    ...
    expect_true(fit$success)
    expect_true(formula_fit$success)
    expect_true(all(is.finite(c(fit$mu_delta_ci, fit$alpha_delta_ci))))
    expect_true(all(is.finite(c(
      formula_fit$mu_delta_ci,
      formula_fit$alpha_delta_ci
    ))))
    expect_equal(fit$data$A, case$A)
    expect_equal(fit$data$R, case$R)
    expect_equal(formula_fit$data$R, case$R)
  }
})
...
test_that("parametric simultaneous confidence helpers work for unadjusted fits", {
  ...
  parametric_reference <- TruncComp:::prepareParametricJointReference(fit$data)
  fitted_log_or <- log(as.numeric(fit$alpha_delta))

  expect_equal(
    TruncComp:::parametricJointCandidate(parametric_reference, 0, 0)$statistic,
    fit$statistic,
    tolerance = 1e-10
  )
  expect_equal(
    TruncComp:::parametricJointCandidate(
...
test_that("parametric simultaneous helpers derive default offsets from fitted data", {
  ...
  offsets <- TruncComp:::jointContrastDefaultOffsets(fit)
...
test_that("parametric simultaneous surface matches direct constrained model fits", {
  ...
  for(case in cases) {
    ...
    parametric_reference <- TruncComp:::prepareParametricJointReference(case$data)
...
test_that("adjusted formula interface stores adjustment metadata and conditional outputs", {
  ...
  expect_equal(unname(ci["alpha_delta", ]), fit$alpha_delta_ci)
  fitted_log_or <- log(as.numeric(fit$alpha_delta))
  joint <- joint_contrast_surface(
    fit,
    mu_delta = c(0, fit$mu_delta),
    log_or_delta = c(0, fitted_log_or),
    plot = FALSE
  )
  expect_equal(joint$surface[1, 1], fit$statistic, tolerance = 1e-8)
  expect_equal(joint$surface[2, 2], 0, tolerance = 1e-8)

  candidate_mu <- fit$mu_delta + 0.2
  candidate_log_or <- fitted_log_or - 0.15
  candidate_surface <- joint_contrast_surface(
    fit,
    mu_delta = candidate_mu,
    log_or_delta = candidate_log_or,
    plot = FALSE
  )
  glm_alt <- suppressWarnings(stats::glm(
    A ~ R + L1 + L2,
  ...
  glm_constrained <- suppressWarnings(stats::glm(
    ...
    family = stats::binomial(),
    data = fit$data,
    offset = candidate_log_or * R
  ))
  observed <- droplevels(fit$data[fit$data$A == 1, , drop = FALSE])
  lm_alt <- stats::lm(Y ~ R + L1 + L2, data = observed)
  lm_constrained <- stats::lm(
    Y ~ L1 + L2,
    data = observed,
    offset = candidate_mu * R
  )
  direct_statistic <- 2 * (
    as.numeric(stats::logLik(glm_alt)) - as.numeric(stats::logLik(glm_constrained)) +
      as.numeric(stats::logLik(lm_alt, REML = FALSE)) -
      as.numeric(stats::logLik(lm_constrained, REML = FALSE))
  )
  expect_equal(candidate_surface$surface[1, 1], direct_statistic, tolerance = 1e-8)

  capture.output(joint_from_confint <- confint(
    fit,
    parameter = "joint",
    mu_delta = fit$mu_delta,
    log_or_delta = fitted_log_or,
    plot = FALSE
  ))
  expect_equal(joint_from_confint$surface[1, 1], 0, tolerance = 1e-8)
...
test_that("adjusted parametric joint surfaces preserve transformed formula environments", {
  case <- adjusted_lrt_case(202605211, 18)
  case$positive_covariate <- exp(case$L1)

  fit <- local({
    custom_log <- function(x) log(x)
    trunc_comp(
      Y ~ R,
      atom = 0,
      data = model_data_for_formula(
        case,
        adjust = ~ custom_log(positive_covariate) + L2
      ),
      method = "lrt",
      adjust = ~ custom_log(positive_covariate) + L2
    )
  ...
  expect_true(fit$success)
  expect_equal(fit$adjust, "custom_log(positive_covariate) + L2")
  expect_s3_class(fit$adjust_formula, "formula")

  fitted_log_or <- log(as.numeric(fit$alpha_delta))
  surface <- joint_contrast_surface(
    fit,
    mu_delta = c(0, fit$mu_delta),
    log_or_delta = c(0, fitted_log_or),
    plot = FALSE
  )
  expect_equal(surface$surface[1, 1], fit$statistic, tolerance = 1e-8)
  expect_equal(surface$surface[2, 2], 0, tolerance = 1e-8)

  candidate_mu <- fit$mu_delta + 0.15
  candidate_log_or <- fitted_log_or - 0.1
  candidate_surface <- joint_contrast_surface(
    fit,
    mu_delta = candidate_mu,
    log_or_delta = candidate_log_or,
  ...
  formulas <- TruncComp:::parametric_model_formulas(fit$adjust_formula)
  glm_alt <- suppressWarnings(stats::glm(
    formulas$bernoulli_alt,
    family = stats::binomial(),
    data = fit$data
...
test_that("observed-only factor levels are dropped before adjusted observed-outcome fits", {
  ...
  fits <- TruncComp:::parametric_fit_models(data, adjust = ~ L)
  ...
  el_design <- TruncComp:::el_regression_design(fits$formulas$normal_alt, fits$normal_data)
...
test_that("LRT boundary cases keep the statistic when profile intervals are undefined", {
