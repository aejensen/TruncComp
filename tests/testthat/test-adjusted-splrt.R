...
splrt_logistic_profile_statistics <- function(data, fit, adjust = NULL) {
  ...
  vapply(log(fit$alpha_delta_ci), function(value) {
...
expect_splrt_logistic_profile_endpoints <- function(data, fit, adjust = NULL,
                                                    conf.level = 0.95,
                                                    tolerance = 1e-5) {
  statistics <- splrt_logistic_profile_statistics(data, fit, adjust = adjust)
  critical_value <- stats::qchisq(conf.level, df = 1)

  expect_equal(statistics, rep(critical_value, 2), tolerance = tolerance)
...
adjusted_splrt_reference <- function(data, adjust, conf.level = 0.95) {
  ...
  glm_alt <- suppressWarnings(stats::glm(formulas$bernoulli_alt,
                                         family = stats::binomial(),
                                         data = data))
  el_fit <- TruncComp:::el_regression_fit(observed,
                                           formulas$normal_alt,
                                           mu = 0,
...
direct_profile_statistic <- function(design, delta) {
  ...
  if(nuisance_count == 0) {
    beta <- TruncComp:::el_regression_beta(delta, numeric(0), design$term_index, ncol(design$X))
    return(direct_dual_statistic(TruncComp:::el_regression_moments(design, beta)))
  }

  nuisance_start <- TruncComp:::el_regression_restricted_ls(design, delta)
  fit <- stats::optim(
    nuisance_start,
    fn = function(nuisance) {
      beta <- TruncComp:::el_regression_beta(delta, nuisance, design$term_index, ncol(design$X))
      moments <- TruncComp:::el_regression_moments(design, beta)
...
test_that("adjust equals ~1 reproduces the unadjusted SPLRT fit", {
  ...
  expect_splrt_logistic_profile_endpoints(unadjusted$data, unadjusted)
  expect_splrt_logistic_profile_endpoints(intercept_only$data, intercept_only)
})
...
test_that("adjusted SPLRT matches direct logistic and regression-EL references", {
  ...
  for(case in cases) {
    ...
    expect_splrt_logistic_profile_endpoints(case$data, fit, adjust = case$adjust)
...
test_that("regression EL profile matches a direct combined optimization on a small case", {
  ...
  design <- TruncComp:::el_regression_design(Y ~ R + L, observed)
  delta <- design$estimate + 0.2
  profile <- TruncComp:::el_regression_profile_fit(design, delta)
...
test_that("adjusted SPLRT handles the packaged adjusted example and attenuates after adjustment", {
  ...
  expect_splrt_logistic_profile_endpoints(fit_unadjusted$data, fit_unadjusted)
  expect_splrt_logistic_profile_endpoints(fit_adjusted$data, fit_adjusted, adjust = ~ L)
  ...
  expect_error(confint(fit_adjusted, parameter = "delta", method = "profile"),
               "not implemented for adjusted fits")
  fitted_log_or <- log(as.numeric(fit_adjusted$alpha_delta))
  joint <- joint_contrast_surface(
    fit_adjusted,
    mu_delta = c(0, fit_adjusted$mu_delta),
    log_or_delta = c(0, fitted_log_or),
    plot = FALSE
  )
  expect_equal(joint$surface[1, 1], fit_adjusted$statistic, tolerance = 1e-7)
  expect_equal(joint$surface[2, 2], 0, tolerance = 1e-7)

  candidate_mu <- fit_adjusted$mu_delta + 0.15
  ...
  formulas <- TruncComp:::parametric_model_formulas(~ L)
  glm_alt <- suppressWarnings(stats::glm(
    formulas$bernoulli_alt,
    family = stats::binomial(),
    data = fit_adjusted$data
  ))
  ...
  observed <- droplevels(fit_adjusted$data[fit_adjusted$data$A == 1, , drop = FALSE])
  design <- TruncComp:::el_regression_design(formulas$normal_alt, observed)
  direct_statistic <- 2 * (
    as.numeric(stats::logLik(glm_alt)) - as.numeric(stats::logLik(glm_constrained))
  ) + direct_profile_statistic(design, candidate_mu)
  expect_equal(candidate_surface$surface[1, 1], direct_statistic, tolerance = 1e-5)

  capture.output(joint_from_confint <- confint(
    fit_adjusted,
    parameter = "joint",
    mu_delta = fit_adjusted$mu_delta,
    log_or_delta = fitted_log_or,
    plot = FALSE
  ))
  expect_equal(joint_from_confint$surface[1, 1], 0, tolerance = 1e-7)
})

test_that("adjusted SPLRT joint surfaces preserve transformed formula environments", {
  case <- adjusted_splrt_case(2026041531, 18, adjust = ~ L1 + L2)
  ...
  fit <- local({
    ...
    trunc_comp(
      Y ~ R,
      atom = 0,
      data = case[, c("Y", "R", "positive_covariate", "L2")],
      method = "splrt",
  ...
  expect_equal(surface$surface[1, 1], fit$statistic, tolerance = 1e-6)
  expect_equal(surface$surface[2, 2], 0, tolerance = 1e-6)
})
...
test_that("adjusted SPLRT fails cleanly on non-regular adjusted fits", {
  ...
  expect_true(all(is.na(aliased_fit$alpha_delta_ci)))
  ...
  expect_true(all(is.na(separation_fit$alpha_delta_ci)))
