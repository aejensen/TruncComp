#' Internal semiparametric likelihood ratio helpers
#'
#' Developer-facing documentation for the high-level semiparametric fitting
#' wrappers used by `method = "splrt"`.
#'
#' @name trunccomp-splrt-helpers
#' @title Internal semiparametric likelihood ratio helpers
#' @description
#' These helpers combine empirical likelihood inference for the conditional
#' non-atom mean contrast with logistic likelihood ratio inference for the
...
#'
#' Fits the adjusted semiparametric procedure. `data` is the standardized
...
#' delegates the non-atom component to `el_regression_fit()`. Its role
...
#' atom = NULL, call = NULL)`
#'
#' Main semiparametric estimator called by `trunc_comp_core()`. In the
#' unadjusted case it extracts the non-atom outcomes by arm, runs the
#' two-sample empirical likelihood test with `el_mean_diff_fit()`, fits the
#' logistic non-atom probability models with `glm()`, and adds the two component
...
#' semiparametric implementation point to the rest of the package.
#'
...
#' used by the semiparametric path and its adjusted regression extension.
#'
#' @name trunccomp-empirical-likelihood-helpers
...
#' preserve a standard R testing-object shape while providing the non-atom
#' component for the unadjusted `SPLRT` path.
#'
#' ### `el_regression_design(formula, data, term = "R")`
#'
#' Constructs the design object for the adjusted empirical likelihood regression
#' extension. `formula` is typically the non-atom mean model, `data` is the
#' non-atom subset, and `term` names the treatment coefficient of
...
#' ### `el_regression_beta(delta, nuisance, term_index, p)`
#'
#' Reconstructs the full regression coefficient vector when the treatment contrast
...
#' coefficients at a fixed treatment contrast `delta`. The helper returns the
...
#' per non-atom outcome and one column per regression parameter. Its role is to
...
#' treatment contrast `delta`. The helper converts profile failures to `Inf`.
...
#' `estimate` is the unrestricted treatment contrast, `crit` is the chi-squared
...
#' Its role is to provide the adjusted semiparametric marginal interval.
#'
...
#' treatment contrast around `mu`. The helper returns a success/error list with
