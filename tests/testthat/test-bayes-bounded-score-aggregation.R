bounded_score_aggregation_fixture <- function(heaping = "arm_specific") {
  ...
  support_options <- TruncComp:::bayes_normalize_support_options(
  ...
  prior <- TruncComp:::normalize_bayes_prior(
    NULL,
    continuous_support = "bounded_score"
  )
  observed <- data[data$A == 1L, , drop = FALSE]
  y_obs_index <- TruncComp:::bayes_score_grid_index(
    observed$Y,
  ...
  list(
    ...
    standata = TruncComp:::build_bayes_standata(
      data = data,
      atom = -1,
...
test_that("bounded score reporting grids exactly tile the score range", {
  expect_equal(
    TruncComp:::bayes_validate_heaping_grids(
      heaping_grids = c(10, 1, 5),
      score_step = 1,
      score_min = 0,
      score_max = 100
    ),
  ...
  expect_error(
    ...
    "integer multiple of every value in heaping_grids",
    fixed = TRUE
  )
  expect_error(
    TruncComp:::bayes_normalize_support_options(
      continuous_support = "bounded_score",
      score_min = 0,
      score_max = 100,
      score_step = 1,
