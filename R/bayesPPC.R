...
#' non-atom parts of a successful Bayesian two-part fit. The atom p value is
#' based on the maximum armwise absolute deviation between the observed atom
#' rate and the fitted atom probability. The non-atom p value is based on the
...
#' checks the conditional non-atom mixture model for outcomes outside the atom.
...
posterior_predictive_check <- function(object,
  ...
  atom_plot <- bayesplot::ppc_bars_grouped(
    ...
    ggplot2::labs(
      ...
      x = paste0("Atom indicator (atom = ", object$atom, ")"),
      y = "Proportion"
    )
