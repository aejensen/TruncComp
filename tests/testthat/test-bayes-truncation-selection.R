test_that("Bayesian mixture-component ladder doubles up to the requested maximum", {
  expect_equal(
    TruncComp:::bayes_mixture_component_ladder(10, 40),
    c(10L, 20L, 40L)
  )
  expect_equal(
    TruncComp:::bayes_mixture_component_ladder(6, 20),
    c(6L, 12L)
  )
  expect_equal(
    TruncComp:::bayes_mixture_component_ladder(8, 8),
    8L
  )
})

test_that("Truncation acceptance requires clean sampler diagnostics", {
  expect_true(TruncComp:::bayes_truncation_ok(
    convergence_ok = TRUE,
    divergences = 0L
  ))

  expect_false(TruncComp:::bayes_truncation_ok(
...
test_that("Truncation parameter names select alpha and final retained stick weights", {
  expect_equal(
    TruncComp:::bayes_truncation_parameter_names(10),
...
test_that("Auto-selection stores the attempted truncation path and final retained H", {
  fake_fit <- function(mixture_components, settings, diagnostic_ok) {
    ...
    truncation_variables <- TruncComp:::bayes_truncation_parameter_names(
      mixture_components
    )
    diagnostics <- list(
      divergences = 0L,
    ...
    TruncComp:::new_trunc_comp_bayes_fit(
  ...
  fit_auto <- TruncComp:::fit_trunc_comp_bayes(
    data = data,
    atom = 0,
  ...
  fit_fixed <- TruncComp:::fit_trunc_comp_bayes(
    data = data,
    atom = 0,
    mixture_components = 10,
