...
test_that("Bayesian PPC helper returns replicated draws with the expected shapes", {
  ppc_data <- TruncComp:::bayes_ppc_data(
    bayes_ppc_fit,
    ndraws = 7,
    seed = 11
  )
...
test_that("Bayesian PPC helper is reproducible with a fixed seed and clips ndraws", {
  data_first <- TruncComp:::bayes_ppc_data(
    bayes_ppc_fit,
    ndraws = 8,
    seed = 99
  )
  data_second <- TruncComp:::bayes_ppc_data(
    bayes_ppc_fit,
    ndraws = 8,
    seed = 99
  )
  ...
  clipped <- TruncComp:::bayes_ppc_data(
    bayes_ppc_fit,
...
test_that("Positive-support PPC plot inputs are formed on the log scale", {
  ...
  plot_inputs <- TruncComp:::bayes_ppc_continuous_plot_inputs(
    ppc_data,
...
test_that("Bayesian PPC discrepancy summaries match the implemented formulas", {
  ppc_summary <- TruncComp:::bayes_ppc_summary(
    bayes_ppc_fit,
    ndraws = 6,
    seed = 11
  )
  ...
  parameters <- TruncComp:::bayes_ppc_component_parameters(bayes_ppc_fit)
  ...
  for(s in seq_along(ppc_data$draw_indices)) {
    ...
    atom_obs[s] <- TruncComp:::bayes_ppc_atom_discrepancy(
      y_atom = ppc_data$y_atom,
      arm_full = ppc_data$arm_full,
      rho_draw = parameters$rho[draw, ]
    )
    atom_rep[s] <- TruncComp:::bayes_ppc_atom_discrepancy(
      y_atom = ppc_data$yrep_atom[s, ],
      arm_full = ppc_data$arm_full,
      rho_draw = parameters$rho[draw, ]
    )
    cont_obs[s] <- TruncComp:::bayes_ppc_continuous_discrepancy(
    ...
    cont_rep[s] <- TruncComp:::bayes_ppc_continuous_discrepancy(
...
test_that("Positive-support PPC p-values use the log-scale continuous discrepancy", {
  ...
  parameters <- TruncComp:::bayes_ppc_component_parameters(bayes_positive_ppc_fit)

  manual_discrepancy <- TruncComp:::bayes_ppc_continuous_discrepancy(
    y_cont = ppc_data$y_cont,
    arm_obs = ppc_data$arm_obs,
...
test_that("Bounded-continuous PPC draws are bounded and reproducible", {
  first <- TruncComp:::bayes_ppc_data(
    bayes_bounded_continuous_ppc_fit,
    ndraws = 6,
    seed = 31
  )
  second <- TruncComp:::bayes_ppc_data(
    bayes_bounded_continuous_ppc_fit,
    ndraws = 6,
    seed = 31
  )
...
test_that("Bounded-score PPC draws stay on reported support and use discrete labels", {
  ...
  ppc_data_again <- TruncComp:::bayes_ppc_data(
    bayes_bounded_score_ppc_fit,
    ndraws = 6,
    seed = 41
  )
