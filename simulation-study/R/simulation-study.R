...
simulation_study_output_dir <- function(repo_root) {
  ensure_dir(file.path(simulation_study_root(repo_root), "results", "trunccomp-study"))
}
...
load_local_trunccomp <- function(repo_root) {
  ...
  if (requireNamespace("pkgload", quietly = TRUE)) {
    loaded <- tryCatch(
      ...
      error = function(e) {
        message(
          "Unable to load the full local TruncComp package; ",
    ...
    if (loaded) {
      options(trunccomp.simulation_loader = "pkgload")
  ...
  if (length(missing_files)) {
    stop(
      "Unable to load the frequentist TruncComp sources. Missing files: ",
  ...
  options(trunccomp.simulation_loader = "source")

  invisible(TRUE)
}

simulation_study_simulate_truncated_data <- function(...) {
  if (identical(getOption("trunccomp.simulation_loader"), "source")) {
    return(simulate_truncated_data(...))
  }

  TruncComp::simulate_truncated_data(...)
}
...
simulation_study_default_config <- function(reps = 25000L,
  ...
  list(
    version = "trunccomp-simulation-study-v1",
...
simulation_study_scenarios <- function() {
  list(
    S1 = list(
      scenario_id = "S1",
      scenario_order = 1L,
      short_label = "S1 Normal non-atom shift",
      title = "Normal non-atom shift with low atom probability",
      purpose = paste(
        "Baseline parametric reference where the conditional non-atom distribution is Normal,",
        "the atom probability is unchanged, and the two likelihood ratio methods should track each other closely."
      ),
      atom_latex = "$\\pi_{0,h} = \\pi_{1,h} = 0.85$",
      ...
      effect_parameters = function(h) {
        ...
        list(
          ...
          effect_text = sprintf("$\\\\delta_h = %.2f$", delta),
          survivor_mean0 = 3,
    ...
    S2 = list(
      scenario_id = "S2",
      scenario_order = 2L,
      short_label = "S2 Probability-only effect",
      title = "Probability-only effect",
      purpose = paste(
        "Two-degree-of-freedom penalty case where treatment acts only through the binary observation component."
      ),
      atom_latex = "$\\pi_{0,h} = 0.45$, $\\pi_{1,h} \\in \\{0.45, 0.50, 0.55, 0.60\\}$",
      ...
      effect_parameters = function(h) {
        ...
        list(
          ...
          effect_text = sprintf("$\\\\pi_{1,h} = %.2f$", pi1),
          survivor_mean0 = 3,
          survivor_mean1 = 3,
    ...
    S3 = list(
      ...
      purpose = paste(
        "Signature case where the non-atom probability and conditional non-atom mean move in opposite directions while the combined-outcome mean stays constant."
      ),
      atom_latex = "$\\pi_{0,h} = 0.50$, $\\pi_{1,h} \\in \\{0.50, 0.525, 0.55, 0.60\\}$",
      survivor_latex = paste(
        "$Y \\mid A = 1, R = 0 \\sim N(3, 1)$,",
        "$Y \\mid A = 1, R = 1 \\sim N(1.5 / \\pi_{1,h}, 1)$"
      ),
      effect_parameters = function(h) {
        ...
        list(
          ...
          effect_text = sprintf("$\\\\pi_{1,h} = %.3f$, $1.5 / \\\\pi_{1,h} = %.3f$", pi1, treatment_mean),
          survivor_mean0 = 3,
    ...
    S4 = list(
      ...
      atom_latex = "$\\pi_{0,h} = 0.50$, $\\pi_{1,h} \\in \\{0.50, 0.55, 0.60, 0.65\\}$",
      survivor_latex = paste(
        "$Y \\mid A = 1, R = 0 \\sim N(3, 1)$,",
      ...
      effect_parameters = function(h) {
        ...
        list(
          ...
          f0 = function(n) stats::rnorm(n, 3, 1),
          f1 = function(n) stats::rnorm(n, treatment_mean, 1),
          effect_text = sprintf("$\\\\pi_{1,h} = %.2f$, $3 + 0.15h = %.2f$", pi1, treatment_mean),
          survivor_mean0 = 3,
          survivor_mean1 = treatment_mean,
    ...
    S5 = list(
      ...
      purpose = paste(
        "A non-atom case with concurrent mean, variance, and skewness changes designed to separate semiparametric and parametric performance."
      ),
      atom_latex = "$\\pi_{0,h} = \\pi_{1,h} = 0.65$",
      survivor_latex = paste(
      ...
      effect_parameters = function(h) {
        ...
        list(
          ...
          effect_text = sprintf("$k_h = %.1f$, $m_h = %.1f$", shape1, mean1),
          survivor_mean0 = 3,
    ...
    S6 = list(
      ...
      purpose = paste(
        "Mixture case with rare extreme non-atom outcomes that is deliberately friendly to rank-based robustness."
      ),
      atom_latex = "$\\pi_{0,h} = \\pi_{1,h} = 0.80$",
      survivor_latex = paste(
        "$Y \\mid A = 1, R = r$ is",
...
.simulation_study_write_rds_atomic <- function(object, path) {
  tmp_path <- tempfile(pattern = "trunccomp-sim-", tmpdir = dirname(path), fileext = ".rds")
...
simulation_study_run_cell <- function(cell, output_dir) {
  ...
  result <- list(
    version = "trunccomp-simulation-study-cell-v1",
