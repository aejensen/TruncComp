test_that("bounded_kernel dispatch and prior validation keep beta defaults", {
  ...
  expect_equal(
    TruncComp:::bayes_model_name("bounded_continuous"),
    "trunc_comp_bayes_bounded_continuous"
  )
  expect_equal(
    TruncComp:::bayes_model_name("bounded_score"),
    "trunc_comp_bayes_bounded_score"
  )
  expect_equal(
    TruncComp:::bayes_model_name("bounded_continuous", bounded_kernel = "logit_normal"),
    "trunc_comp_bayes_bounded_continuous_logit_normal"
  )
  expect_equal(
    TruncComp:::bayes_model_name("bounded_score", bounded_kernel = "logit_normal"),
  ...
  logit_prior <- TruncComp:::normalize_bayes_prior(
    NULL,
  ...
  beta_prior <- TruncComp:::normalize_bayes_prior(
    NULL,
    continuous_support = "bounded_score"
  )
...
test_that("bounded logit-normal fits return successful objects and settings", {
  ...
  expect_true(all(TruncComp:::bayes_parameter_names("all") %in% names(bayes_bounded_continuous_logit_fit$draws)))
  expect_true(all(TruncComp:::bayes_parameter_names("all") %in% names(bayes_bounded_score_logit_fit$draws)))
...
test_that("bounded-continuous logit-normal means agree with generated quantities", {
  ...
  unit_mean_0 <- matrix(
    mapply(TruncComp:::bayes_logitnormal_unit_mean, as.vector(mu_0), as.vector(sigma_0)),
    nrow = nrow(mu_0),
    ncol = ncol(mu_0)
  )
  unit_mean_1 <- matrix(
    mapply(TruncComp:::bayes_logitnormal_unit_mean, as.vector(mu_1), as.vector(sigma_1)),
    nrow = nrow(mu_1),
    ncol = ncol(mu_1)
  )
...
test_that("PPC and posterior density helpers support bounded logit-normal kernels", {
  continuous_ppc <- TruncComp:::bayes_ppc_data(
    bayes_bounded_continuous_logit_fit,
    ndraws = 6,
    seed = 51
  )
  score_ppc <- TruncComp:::bayes_ppc_data(
  ...
  continuous_plot_data <- TruncComp:::bayes_density_plot_data(
    bayes_bounded_continuous_logit_fit,
    n = 30
  )
  score_plot_data <- TruncComp:::bayes_density_plot_data(
    bayes_bounded_score_logit_fit,
    n = 30
  )
