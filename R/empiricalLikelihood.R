...
el_regression_design <- function(formula, data, term = "R") {
  ...
  if(is.na(term_index)) {
    return(list(success = FALSE,
                error = paste0("The treatment term '", term, "' is not identifiable in the non-atom design.")))
  }

  qr_x <- qr(X)
  if(qr_x$rank < ncol(X)) {
    return(list(success = FALSE,
                error = "The non-atom design matrix is rank deficient."))
  }

  if(nrow(X) <= ncol(X)) {
    return(list(success = FALSE,
                error = "There are too few non-atom outcomes relative to the adjusted design."))
  }
  ...
  if(length(nuisance_start) == 0) {
    ...
    if(!isTRUE(dual$success)) {
      return(list(success = FALSE,
                  feasible = FALSE,
                  statistic = Inf,
                  error = "Unable to solve the empirical likelihood dual problem for the constrained treatment contrast."))
    }
...
el_regression_fit <- function(data, formula, term = "R", mu = 0, conf.level = 0.95, tol = 1e-8) {
  ...
  if(!is.finite(statistic)) {
    return(list(success = FALSE,
                error = "Adjusted semiparametric empirical likelihood is not estimable under the supplied covariates."))
  }
