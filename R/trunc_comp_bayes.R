#' Fit the experimental Bayesian two-part comparison model
#'
#' Fits an explicit two-part Bayesian model for a binary treatment comparison
#' with a distinguished atom representing a substantive event. The Bayesian
#' analysis is unadjusted and supports real-valued, strictly positive, bounded
#' continuous, and bounded reported score non-atom outcomes.
#'
...
#'   is the encoded endpoint vector.
#' @param a Binary indicator for whether the endpoint is outside the atom in the
#'   default interface.
...
#'   exactly one unique finite value. The encoded endpoint must satisfy
#'   `y[a == 0] == atom` and `y[a == 1] != atom`.
...
#' @param continuous_support Support of the non-atom component.
#'   `"real_line"` fits a Gaussian-mixture model on the real line,
...
#'   positive. `"bounded_continuous"` fits a Beta-mixture model by default for
#'   finely measured bounded outcomes inside `(score_min, score_max)`.
#'   `"bounded_score"` fits a discretized and heaped Beta-mixture model by
#'   default for integer or grid-valued
...
#'   `"beta"` selects bounded Beta-mixture models and is the default.
#'   `"logit_normal"` selects logit-normal mixture kernels for
#'   `continuous_support = "bounded_continuous"` and
#'   `continuous_support = "bounded_score"`.
#' @param mixture_components Initial number of retained components in each
#'   arm-specific stick-breaking mixture. Must be at least `2`. The default is
#'   `10`.
#' @param auto_select_mixture_components Logical flag indicating whether the
#'   Bayesian fit should automatically increase the finite mixture component
#'   count by doubling it until sampler diagnostics pass or the maximum count
#'   is reached. This procedure responds to MCMC diagnostics and does not assess
#'   finite-mixture approximation error.
#' @param mixture_components_max Optional maximum component count used when
...
#' @param control Stan control list. Supported entries are `adapt_delta` and
#'   `max_treedepth`.
...
#'   component-refitting settings and history, the standardized analysis data,
...
#' probability per treatment arm and one finite stick-breaking mixture per
#' treatment arm for the non-atom outcomes. The non-atom component can be
#' modeled with Gaussian kernels on the real line, Gamma kernels on the
#' positive real line, or bounded Beta-mixture or logit-normal
#' kernels.
#' Likelihood ratio statistics are not reported, but discrepancy-based
#' posterior predictive p values are available through
#' [posterior_predictive_pvalues()] and [posterior_predictive_check()] for model
#' checking.
#'
#' The `mixture_components` argument controls the initial number of retained
#' stick-breaking components. When
...
#' diagnostics pass. If sampler diagnostics fail, the component count is
...
#' as the final fit.
#' This rule is a sampler-diagnostic refitting policy. It does not estimate the
#' residual stick mass or otherwise assess finite-mixture approximation error.
#'
#' In the stored posterior draws and summaries:
#'
#' - `rho_0` and `rho_1` are the arm-specific atom probabilities.
#' - `pi_0` and `pi_1` are the arm-specific probabilities of being outside the
#'   atom, so `pi_r = 1 - rho_r`.
#' - `mu_0_c` and `mu_1_c` are the arm-specific conditional non-atom means.
...
#'   treatment-to-control odds ratio for being outside the atom.
#'
#' The coded endpoint mean difference is
#'
#' `delta = [atom * rho_1 + pi_1 * mu_1^c] - [atom * rho_0 + pi_0 * mu_0^c]`,
#'
#' which combines the atom probability and the conditional non-atom mean using
#' the specified numerical atom code.
#' @examples
#' \dontrun{
#' library(TruncComp)
...
trunc_comp_bayes.formula <- function(formula, atom, data,
  ...
  non_atom <- as.integer(outcome != atom)
