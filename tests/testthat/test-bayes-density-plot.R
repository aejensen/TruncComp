  ...
  if(identical(support, "positive_real")) {
    ...
    return(t(vapply(
      ...
      function(draw) {
        ...
        kernels <- vapply(
          seq_along(weights),
          function(h) TruncComp:::bayes_positive_kernel_density(x, mean = means[h], shape = shapes[h]),
          numeric(length(x))
        )
...
test_that("Bayesian density helper respects x, summarizes densities, and responds to conf.level", {
  ...
  data_default <- TruncComp:::bayes_density_plot_data(
    bayes_density_plot_fit,
    x = NULL,
    n = 120
  )
  data_80 <- TruncComp:::bayes_density_plot_data(
    bayes_density_plot_fit,
    x = x_grid,
    conf.level = 0.80
  )
  data_95 <- TruncComp:::bayes_density_plot_data(
    bayes_density_plot_fit,
    x = x_grid,
    conf.level = 0.95
  )
...
test_that("Bayesian density helper builds a usable default grid for degenerate observed ranges", {
  ...
  plot_data <- TruncComp:::bayes_density_plot_data(
    degenerate_fit,
    x = NULL,
    n = 5
  )
...
test_that("posterior_density_plot works for positive-support fits", {
  ...
  default_data <- TruncComp:::bayes_density_plot_data(
