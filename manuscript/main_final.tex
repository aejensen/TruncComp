...
The contribution is a likelihood-ratio framework for testing this two-component null with one prespecified p-value. The test is joint in the atom and non-atom components, but the reporting remains component-resolved: the analysis yields estimates for the atom probability contrast, the non-atom mean contrast, the odds ratio for being outside the atom, and likelihood-ratio confidence regions. A numerical code $e_0$ for the atom can still be used to report an induced combined-outcome contrast, but the primary likelihood-ratio test is not reduced to that one-dimensional scale. This preserves the practical attraction of a single primary p-value while making clear which parts of the observed endpoint drive the result.
...
Because the Gaussian non-atom likelihood and the Bernoulli likelihood depend on disjoint finite-dimensional parameter blocks, profiling the product likelihood at fixed $(\mu_\delta,\beta_\delta)$ separates into the two component profiles in Equation~(\ref{eq:LRTtotal}); the displayed sum is therefore the full observed-data profile likelihood-ratio deviance for the two scalar restrictions.

\begin{proposition}
Assume that the observed-data Gaussian/logistic model above is correctly specified and finite-dimensional, with $s_\beta(x)=h_\beta(x)^\top\eta_\beta$ and $s_\mu(x)=h_\mu(x)^\top\eta_\mu$ for fixed measurable covariate transformations. Assume that the true parameter lies in the interior of the parameter space with $\sigma_0^2>0$, that the model is identifiable under both the unrestricted and null parameter spaces, and that the observed-data likelihood satisfies the standard regularity conditions for Wilks' theorem in a neighborhood of the true parameter, including existence and consistency of the restricted and unrestricted maximum likelihood estimators and nonsingular observed-data Fisher information. Assume also that both treatment arms and both atom/non-atom components carry asymptotic information; for example, under a fixed treatment design, $n_0,n_1\to\infty$ with $n_1/n_0\to\lambda_R\in(0,\infty)$ and the observation probabilities are bounded away from 0 and 1 on the relevant covariate support. Then, under the null hypothesis $\mu_\delta = \beta_\delta = 0$, the profile likelihood-ratio statistic $W_n^P(0,0)$ in Equation~(\ref{eq:LRTtotal}) converges in distribution to $\chi^2_2$.
\end{proposition}

\begin{proof}
The Gaussian non-atom likelihood depends on $(\mu_0,\mu_\delta,\eta_\mu,\sigma^2)$, whereas the Bernoulli likelihood depends on $(\beta_0,\beta_\delta,\eta_\beta)$. These parameter blocks are disjoint. Hence, for fixed $(\mu_\delta,\beta_\delta)$, the supremum of the product likelihood over nuisance parameters equals the product of the two component suprema, and the full profile likelihood-ratio deviance is exactly the sum displayed in Equation~(\ref{eq:LRTtotal}).

Under the stated regularity conditions, the observed-data model is a regular finite-dimensional likelihood model and the null model $\mu_\delta=\beta_\delta=0$ is a smooth submodel imposing two scalar restrictions on an interior point of the parameter space. Wilks' theorem \citep{wilks1938large} applied to the full observed-data likelihood therefore gives
\[
...
\end{equation}
The first term is the profile empirical-likelihood deviance in Equation~(\ref{eq:adjusted-el-profile}), or equivalently Equation~(\ref{eq:nonparametric}) in the no-covariate special case; the second term is the Bernoulli profile likelihood-ratio statistic from Equation~(\ref{eq:LRTtotal}). The same joint null is tested by evaluating $\widetilde{W}_n^P(0,0)$, with the non-atom mean restriction on the mean contrast $\mu_\delta$ and the observation component restriction on the Bernoulli coefficient $\beta_\delta$.
...
\begin{proof}
For the observation component, the stated finite-dimensional regularity conditions give the usual local quadratic expansion of the Bernoulli log-likelihood. Wilks' theorem for the single scalar restriction $\beta_\delta=0$ gives $W_{A,n}^P(0)\overset{d}{\to}\chi^2_1$.

For the non-atom component, positivity implies $M_n/n\to\Pr(A_i=1)>0$ in probability. Under the conditional law given $A_i=1$, the correctly specified mean model gives $\E\{Z_i(Y_i-Z_i^\top\theta_0)\mid A_i=1\}=0$, and the stated moment, covariance, rank, convex-hull, and nuisance-uniqueness conditions are the standard empirical-likelihood regularity conditions for estimating equations with a profiled nuisance parameter. The empirical-likelihood Wilks theorem for the single scalar restriction $\mu_\delta=0$ therefore gives $\widetilde{W}_{Y,n}^P(0)\overset{d}{\to}\chi^2_1$ \citep{owen1991empirical,qin1994empirical}.

It remains to identify the joint limit. Let $S_A(A_i,R_i,X_i)$ denote any centered component of the Bernoulli score evaluated at the true parameter. By the conditional mean-zero relation implied by Equation~(\ref{eq:glm2}), the cross-moment between the Bernoulli score and the empirical-likelihood estimating function is zero:
\[
  \E\!\left[
    S_A(A_i,R_i,X_i)\,
...
Let $g^{(e_0)}$ denote the population analogue of the profiled, standardized map $g_n^{(e_0)}$ for fixed atom code $e_0$.
...
Let $\theta_\delta=(\mu_\delta,\beta_\delta)^\top$ and fix the atom code $e_0$. Let $\theta_{\delta0}$ be the true component contrast and let $\Delta_0=g^{(e_0)}(\theta_{\delta0})$, where $g^{(e_0)}$ is the population analogue of the profiled, standardized map in Equation~(\ref{eq:profiled-delta-map}). Suppose that, in a neighborhood of $\theta_{\delta0}$, the relevant parametric or semi-parametric likelihood-ratio surface has the local expansion
\[
...
\]
uniformly for bounded $h$, and that the relevant profile fibers are nonempty and admit approximate minimizers with probability tending to one. Then
\[
  T_n(\Delta_0)\ \overset{d}{\longrightarrow}\ \chi^2_1,
\]
and the profile confidence set in Equation~(\ref{eq:profile-delta-ci}) has asymptotic coverage $1-\alpha$ for $\Delta_0$. When this set is connected, its endpoint representation is a confidence interval with the same limiting coverage. Moreover, if $\mathcal{C}_{2,n}(1-\alpha)$ is an asymptotic $(1-\alpha)100\%$ confidence region for $\theta_{\delta0}$ and the same root-$n$ approximation to $g_n^{(e_0)}$ holds on that region, then the projected interval in Equation~(\ref{eq:projected-delta-ci}) has at least asymptotic coverage $1-\alpha$ for $\Delta_0$.
\end{proposition}

\begin{proof}
Write $\theta=\theta_{\delta0}+h/\sqrt n$. By the assumed local expansion of the likelihood-ratio surface and the local expansion of the profiled coded-scale map, the constraint $g_n^{(e_0)}(\theta)=\Delta_0$ is, uniformly on bounded local neighborhoods, equivalent up to $o_p(1)$ in the likelihood-ratio objective to the linear constraint $a^\top h=0$, where $a=\nabla g^{(e_0)}(\theta_{\delta0})\neq0$. Hence
\[
  T_n(\Delta_0)
  =
  \inf_{a^\top h=0}
  (h-\widehat h_n)^\top I_\delta(h-\widehat h_n)+o_p(1).
\]
The infimum is the squared $I_\delta$-distance from $\widehat h_n$ to the hyperplane $a^\top h=0$, namely
\[
  \frac{(a^\top \widehat h_n)^2}{a^\top I_\delta^{-1}a}+o_p(1).
\]
Since $\widehat h_n\overset{d}{\to}N(0,I_\delta^{-1})$ and $a\neq0$, this converges in distribution to $\chi^2_1$. The coverage of the profile confidence set follows by inverting this scalar likelihood-ratio statistic and using continuity of the $\chi^2_1$ distribution at $\chi^2_{1,1-\alpha}$.

For the projected interval, on the event $\theta_{\delta0}\in\mathcal C_{2,n}(1-\alpha)$ the image of the component region under the population map contains $\Delta_0$. The stated local approximation of $g_n^{(e_0)}$ makes the corresponding image under the sample profiled map asymptotically equivalent on the root-$n$ component region. Therefore the limiting coverage of the projected interval is at least the limiting coverage of $\mathcal C_{2,n}(1-\alpha)$, namely $1-\alpha$.
\end{proof}
...
Under the asymptotic calibrations described in Section~\ref{sec:method}, the likelihood-ratio procedures test the joint null $\mu_\delta=0$ and $\beta_\delta=0$: no treatment effect on the non-atom EQ-VAS mean and no treatment effect on the odds of being outside the death atom. The parametric LRT statistic was \ApplicationLRTStatistic{} with p-value \ApplicationLRTP{}, and the semi-parametric LRT statistic was \ApplicationSPLRTStatistic{} with p-value \ApplicationSPLRTP{}. In this data set the two procedures agree closely, suggesting that the signal is mainly a location shift in the EQ-VAS component rather than a feature driven by the Gaussian working model. The component summaries have direct observed-data interpretations: $\mu_\delta$ is the rt-PA minus control mean EQ-VAS difference among participants alive with observed scores; $\alpha_\delta$ is the rt-PA/control odds ratio for being alive with observed EQ-VAS; and $\Delta$ is the rt-PA minus control mean of $\widetilde{Y}^{(-1)}$. The joint rejection should therefore be interpreted as evidence that the observed atom probability and non-atom mean summaries differ between randomized arms, not as a principal-stratum causal effect.

For $\Delta=\Delta^{(-1)}$, the direct Welch interval for $\widetilde{Y}^{(-1)}$ was $[-0.46, 4.63]$, and the semi-parametric profile-likelihood endpoint range was almost identical at $[-0.46, 4.62]$. By contrast, projecting the joint confidence region for the atom and non-atom components onto the $\Delta$ scale gave the wider interval $[-1.09, 5.25]$, reflecting the additional uncertainty retained by the two-component likelihood-ratio region.
