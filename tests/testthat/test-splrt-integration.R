splrt_unadjusted_logistic_profile_statistics <- function(model) {
  data <- model$data
  ...
  vapply(log(model$alpha_delta_ci), function(value) {
...
test_that("SPLRT reproduces the TruncComp example outputs without loading EL", {
  ...
  expect_equal(model$alpha_delta_ci, c(0.1660494, 1.5968070), tolerance = 1e-6)
  expect_equal(
    splrt_unadjusted_logistic_profile_statistics(model),
    rep(stats::qchisq(model$conf.level, df = 1), 2),
    tolerance = 1e-5
  )
...
test_that("SPLRT logistic intervals invert the requested likelihood ratio cutoff", {
  model <- trunc_comp(
    Y ~ R,
    atom = 0,
  ...
  expect_equal(
    splrt_unadjusted_logistic_profile_statistics(model),
    rep(stats::qchisq(0.90, df = 1), 2),
    tolerance = 1e-5
  )
  expect_lte(model$alpha_delta_ci[1], as.numeric(model$alpha_delta))
  expect_gte(model$alpha_delta_ci[2], as.numeric(model$alpha_delta))
})

test_that("SPLRT keeps logistic profile intervals undefined at a boundary fit", {
  ...
  expect_true(model$success)
  expect_true(all(is.na(model$alpha_delta_ci)))
})
...
test_that("joint contrast surface matches the null test and vanishes at the fitted point", {
  example_data <- trunc_comp_example
  model <- trunc_comp(Y ~ R, atom = 0, data = example_data, method = "splrt")
  reference <- TruncComp:::prepareSPLRTJointReference(model$data)

  expect_equal(
    TruncComp:::splrtJointCandidate(reference, 0, 0)$statistic,
    model$statistic,
    tolerance = 1e-6
  )
  expect_equal(
    TruncComp:::splrtJointCandidate(
      reference,
      model$mu_delta,
      log(model$alpha_delta)
    )$statistic,
    0,
    tolerance = 1e-6
  )
...
test_that("joint_contrast_surface derives default offsets from fitted SPLRT data", {
  ...
  offsets <- TruncComp:::jointContrastDefaultOffsets(model)
...
test_that("joint contrast plotting helper returns a ggplot object", {
  example_data <- trunc_comp_example
  model <- trunc_comp(Y ~ R, atom = 0, data = example_data, method = "splrt")
  joint <- TruncComp:::jointContrastSurfaceData(model, plot = FALSE, resolution = 5)

  plot_obj <- TruncComp:::jointContrastPlot(
    joint$mu_delta,
    joint$log_or_delta,
    joint$surface,
    model,
...
test_that("cached logistic profile matches the uncached helper", {
  ...
  expect_equal(TruncComp:::logit.LRT.prepared(logit_reference, 0),
               ...
               tolerance = 1e-10)
  expect_equal(TruncComp:::logit.LRT.prepared(logit_reference, log(as.numeric(model$alpha_delta))),
               TruncComp:::logit.LRT(model$data, log(as.numeric(model$alpha_delta))),
               tolerance = 1e-10)
})
...
test_that("cached logistic profile adapts to extreme observation rates", {
  ...
  logit_reference <- TruncComp:::logit.prepare(fit$data)
  wide_profile <- stats::optimize(
    function(b0) TruncComp:::logit.likelihood.prepared(logit_reference, c(b0, 0)),
    interval = c(-20, 20),
    maximum = FALSE
  )

  expect_equal(TruncComp:::logit.likelihood.profile.prepared(logit_reference, 0),
               wide_profile$objective,
               tolerance = 1e-10)
  expect_equal(TruncComp:::logit.LRT.prepared(logit_reference, 0), 0, tolerance = 1e-10)
  reference <- TruncComp:::prepareSPLRTJointReference(fit$data)
  expect_equal(
    TruncComp:::splrtJointCandidate(
      reference,
      fit$mu_delta,
      log(as.numeric(fit$alpha_delta))
    )$statistic,
    0,
    tolerance = 1e-10
  )
