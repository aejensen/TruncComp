functions {
  real beta_interval_log(real lower, real upper, real beta_a, real beta_b) {
    if (upper <= lower) {
    ...
    if (lower <= 0) {
      return beta_lcdf(upper | beta_a, beta_b);
    }

    if (upper >= 1) {
      return beta_lccdf(lower | beta_a, beta_b);
    }

    return log_diff_exp(
      beta_lcdf(upper | beta_a, beta_b),
      beta_lcdf(lower | beta_a, beta_b)
    );
  }
}
