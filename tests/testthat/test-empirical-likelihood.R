...
test_that("regression empirical likelihood is zero at the unconstrained OLS coefficient", {
  ...
  profile <- TruncComp:::el_regression_profile_fit(design, design$estimate)
...
test_that("regression empirical likelihood is finite and non-negative on regular cases", {
  ...
  design <- TruncComp:::el_regression_design(Y ~ R + L, observed)
  statistic <- TruncComp:::el_regression_statistic(design, design$estimate + 0.25)
...
test_that("regression empirical likelihood reduces to the two-sample mean-difference EL without covariates", {
  ...
  mean_fit <- TruncComp:::el_mean_diff_fit(x, y)
  regression_fit <- TruncComp:::el_regression_fit(observed, Y ~ R, mu = 0)
