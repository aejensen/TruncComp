...
The output reports estimates of the mean difference among observed outcomes, $\mu_\delta$, and the odds ratio $\exp(\beta_\delta)$ of being observed. It also reports marginal 95\% confidence intervals for the two components, together with the joint likelihood-ratio statistic and its p-value for the null hypothesis of no treatment effect. For successful unadjusted fits from either method, \texttt{confint(..., type = "marginal")} also reports \texttt{DeltaMarginalCI} and \texttt{DeltaProfileCI}; \texttt{confint(..., type = "delta\_projected")} returns \texttt{DeltaProjectedCI}; and \texttt{confint(..., type = "simultaneous")} or \texttt{jointContrastCI(fit)} evaluates the two-dimensional simultaneous region for $(\mu_\delta,\beta_\delta)$, typically visualized on the $(\mu_\delta,\log \text{OR})$ scale. Because \texttt{DeltaProjectedCI} is inherited from that full simultaneous region, it is usually more conservative than the one-dimensional profile interval \texttt{DeltaProfileCI}.
...
\begin{lstlisting}
confint(fit_splrt, type = "marginal")
confint(fit_splrt, type = "delta_profile")
confint(fit_splrt, type = "delta_projected")
confint(fit_splrt, type = "simultaneous", plot = TRUE, offset = 1.4, resolution = 50)
...
confint(fit_adjusted, type = "marginal")
\end{lstlisting}
...
For the current build, the local Dryad ARDS data did not yield the intended different-conclusion case, so the pipeline falls back to \ApplicationDatasetName{} rather than the originally planned COVID-STEROID 2 subgroup. The semi-parametric fit reports $\mu_\delta = 1350.86$ with 95\% confidence interval $(-23.66, 4471.96)$ and $\exp(\beta_\delta) = 0.679$ with 95\% confidence interval $(0.438, 1.010)$, together with the joint p-value $0.0266$. The corresponding parametric fit reports $\mu_\delta = 1350.86$ with 95\% confidence interval $(511.18, 2190.54)$ and $\exp(\beta_\delta) = 0.679$ with 95\% confidence interval $(0.448, 1.029)$, together with the joint p-value $0.00120$. These values therefore illustrate the implemented software outputs, but they do not reproduce the COVID-STEROID 2 subgroup definition or its adjustment strategy exactly.
