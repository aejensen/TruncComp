load_local_trunccomp <- function(repo_root) {
  ...
  if (!requireNamespace("TruncComp", quietly = TRUE)) {
    stop(
      paste(
        "The TruncComp package is required to build the manuscript application.",
        "Install it or make the repository-root package available."
      ),
      call. = FALSE
    )
  }
...
.application_ist3_raw_path <- function(manuscript_dir) {
  path <- file.path(.application_data_dir(manuscript_dir), "ist3.dat")
  if (file.exists(path)) {
    return(path)
  }
...
.application_metadata_ist3 <- function(raw, data, raw_path, standardized_path) {
  ...
  list(
    ...
    outcome_label = "death by 6 months plus 6-month EuroQol visual analogue score among participants alive with observed scores",
    ...
    continuous_label = "EuroQol visual analogue score among participants alive at 6 months with observed scores",
