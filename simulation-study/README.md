...
The study uses the active `TruncComp` package at the repository root. When
`pkgload` is available, the drivers load that package directly. Otherwise they
source the required frequentist R files from the same root package.
...
into `simulation-study/results/trunccomp-study/cells/` in this repository and
run:
...
`simulation-study/results/trunccomp-study/`, and, when the full design is
...
```sh
make -C manuscript assets
make -C manuscript pdf
```

The PDF build uses `manuscript/manuscript.tex` and writes
