...
manifest_path <- .parse_flag(args, "--manifest", ci_study_manifest_path(output_dir))
...
if (!file.exists(manifest_path)) {
  stop("No CI-study manifest found at ", manifest_path, call. = FALSE)
}
...
chunk <- manifest[task_id, , drop = FALSE]

load_local_trunccomp2(repo_root)
message(sprintf(
  "Running CI-study manifest task %d: %s, h = %d, n = %d, chunk = %d/%d",
  task_id,
  chunk$scenario_id,
  chunk$h,
  chunk$n,
...
ci_study_run_chunk(chunk, output_dir = output_dir)
