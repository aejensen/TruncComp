...
fit_trunc_comp_bayes_once <- function(data, atom, conf.level = 0.95,
  ...
  if(inherits(truncation_draws, "error") || nrow(truncation_draws) == 0L) {
    return(new_failed_trunc_comp_bayes_fit(
      "Stan sampling completed but the mixture-parameter sampler diagnostics could not be extracted.",
      conf.level = conf.level,
      data = data,
      atom = atom,
      call = call,
...
fit_trunc_comp_bayes <- function(data, atom, conf.level = 0.95,
  ...
  package_model_name <- bayes_model_name(
    continuous_support,
    bounded_kernel = bounded_kernel
  )
  if(!is.null(model_name_override)) {
    if(!(length(model_name_override) == 1L &&
         is.character(model_name_override) &&
         !is.na(model_name_override) &&
         nzchar(model_name_override))) {
      stop("model_name_override must be NULL or a single non-empty character string.")
    }
    if(is.null(model_object)) {
      stop("model_name_override requires an explicit model_object.")
    }
  }
  model_name <- if(is.null(model_name_override)) {
    package_model_name
  } else {
    model_name_override
  }
  ...
  if(is.null(model_object)) {
    model_object <- bayes_package_stanmodel(package_model_name)
  }
  if(is.null(model_object)) {
    return(new_failed_trunc_comp_bayes_fit(
      paste0(
        ...
        package_model_name,
        "' is not available. Reinstall TruncComp after generating Stan exports."
      ),
      conf.level = conf.level,
      data = data,
      atom = atom,
  ...
  for(index in seq_along(candidate_path)) {
    ...
    if(!isTRUE(candidate_fit$success)) {
      ...
      if(index == 1L) {
        ...
        candidate_fit$settings$mixture_selection_note <- if(auto_select_mixture_components) {
          paste0(
            ...
            candidate_H,
            " failed before sampler-diagnostic component refitting could proceed."
          )
        } else {
          paste0(
      ...
      selection_note <- paste0(
        "Sampler-diagnostic component refitting stopped after a failed refit at H = ",
        candidate_H,
  ...
  if(is.null(selected_fit)) {
    ...
    selection_note <- paste0(
      ...
      mixture_components_max,
      " satisfied the mixture-parameter sampler diagnostics. Returning the largest successful fit at H = ",
      selected_fit$settings$mixture_components,
      "."
    )
    selected_fit$diagnostics$truncation_ok <- FALSE
